...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    /* ===== MOBILE DRAWER (FIX: main full width, sidebar overlay) ===== */
    .overlay{
      display:none;
      position:fixed;
      inset:0;
    ...
    @media (max-width: 1024px){
      /* FIX UTAMA: layout selalu 1 kolom, jangan pernah “nyempil” */
      .layout{
    ...
    <aside class="sidebar">
      <div class="sb-brand">
        ...
        <div class="hide-when-collapsed">
          ...
          <div class="sb-sub">Kasir Panel</div>
        </div>
      </div>
...
<a class="sb-item <?= kasir_nav_active('kasir_transactions',$activeMenu) ?>" data-tip="Riwayat" href="kasir_transactions.php">
  <div class="sb-ico">📚</div>
  <div class="sb-text hide-when-collapsed">
    <div class="sb-label">Riwayat</div>
    <div class="sb-desc">Daftar transaksi & struk</div>
  </div>
</a>
        ...
        <a class="sb-item <?= kasir_nav_active('kasir_cash',$activeMenu) ?>" data-tip="Kas" href="kasir_cashdraw.php">
          <div class="sb-ico">💵</div>
          <div class="sb-text hide-when-collapsed">
            <div class="sb-label">Kas Masuk/Keluar</div>
            <div class="sb-desc">Catat uang kas</div>
          </div>
        </a>


        <div class="sb-section hide-when-collapsed">Lainnya</div>
<a class="sb-item <?= kasir_nav_active('kasir_customers',$activeMenu) ?>" data-tip="Pelanggan" href="kasir_customers.php">
  <div class="sb-ico">👥</div>
  <div class="sb-text hide-when-collapsed">
    <div class="sb-label">Pelanggan</div>
    <div class="sb-desc">Daftar & pencarian pelanggan</div>
  </div>
</a>

<a class="sb-item <?= kasir_nav_active('kasir_settings',$activeMenu) ?>" href="kasir_settings.php">
  <div class="sb-ico">⚙️</div>
  <div class="sb-text hide-when-collapsed">
    <div class="sb-label">Pengaturan</div>
    <div class="sb-desc">Printer, tampilan, dll</div>
  </div>
</a>
    ...
    <main class="main">
      <div class="topbar">
        <div class="top-left">
          <!-- 1 tombol ☰: mobile drawer, desktop collapse -->
      ...
      <script>
        ...
        (function(){
          ...
          if (isMobile) return;
          const saved = localStorage.getItem('kasir_sidebar_collapsed');
          if(saved === '1') document.body.classList.add('sb-collapsed');
        })();

        window.__kasirSidebarClose = function(){
          document.body.classList.remove('sb-open');
        };

        // Unified toggle:
        // - mobile: open/close drawer
        // - desktop: collapse/expand (mini)
        window.__kasirSidebarToggle = function(){
          const isMobile = window.matchMedia('(max-width: 1024px)').matches;
          if(isMobile){
            ...
            localStorage.setItem(
              'kasir_sidebar_collapsed',
              document.body.classList.contains('sb-collapsed') ? '1' : '0'
            );
          }
        };

        // Close drawer when click a menu (mobile)
        ...
        // If resizing to mobile, ensure no "collapsed" layout affects widths
