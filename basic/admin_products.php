...
/**
 * Ubah path gambar dari database menjadi URL yang benar untuk halaman di folder /basic.
 */
...
<script>
  ...
  function normalizeBasicImgSrc(path){
    path = (path || '').trim();
  ...
  upload.addEventListener('change', () => {
    if(upload.files && upload.files[0]){
      ...
      show(normalizeBasicImgSrc(preset.value));
    } else {
      show('');
    }
  });
  ...
  preset.addEventListener('change', () => {
    ...
    show(preset.value ? normalizeBasicImgSrc(preset.value) : '');
  });
</script>
