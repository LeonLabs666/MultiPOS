...
$page_title = 'Riwayat Stok • MultiPOS';
$page_h1 = 'Riwayat Stok';
...
// Ambil SEMUA dari stock_movements saja.
// Opname dibedakan via note diawali "Opname #"
$baseSql = "
...
if ($target === 'ingredient') {
  $where .= " AND m.target_type='ingredient' ";
} elseif ($target === 'opname') {
  $where .= " AND m.note LIKE 'Opname #%' ";
} elseif ($target === 'product') {
  $where .= " AND m.target_type='product' ";
}

if ($dir !== 'all') {
  $where .= " AND m.direction = :dir ";
  $params[':dir'] = $dir;
}

if ($q !== '') {
  // cari di nama item (product/ingredient), sku (kalau product), dan note
  $where .= " AND (
      (CASE WHEN m.target_type='product' THEN p.name ELSE i.name END) LIKE :q
      OR (CASE WHEN m.target_type='product' THEN COALESCE(p.sku,'') ELSE '' END) LIKE :q
      OR COALESCE(m.note,'') LIKE :q
    ) ";
  $params[':q'] = '%' . $q . '%';
}

if ($from !== '' && preg_match($dateRe, $from)) {
  $where .= " AND m.created_at >= :from_dt ";
  $params[':from_dt'] = $from . ' 00:00:00';
}
if ($to !== '' && preg_match($dateRe, $to)) {
  $where .= " AND m.created_at <= :to_dt ";
...
$dataSql = "
  SELECT
    ...
    CASE WHEN m.note LIKE 'Opname #%' THEN 'opname' ELSE 'mutasi' END AS source
  {$baseSql} {$where}
